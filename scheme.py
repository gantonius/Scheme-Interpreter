...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        def eval_operand_one_by_one(expr):
            nonlocal env
            return scheme_eval(expr, env)
        evaluated_operands = operands.map(eval_operand_one_by_one)
...
def gen_formals(bindings):
    if bindings is not nil:
        return Pair(bindings.first.first, gen_formals(bindings.second))
    return nil

def gen_vals(bindings, env):
    if bindings is not nil:
        return Pair(scheme_eval(bindings.first.second.first, env), gen_vals(bindings.second, env))
    return nil

def check_form_bindings(bindings):
    if bindings is not nil:
        check_form(bindings.first, 2, 2)
        check_form_bindings(bindings.second)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    check_form_bindings(bindings)
    formals = gen_formals(bindings)
    vals = gen_vals(bindings, env)
    check_formals(formals)
