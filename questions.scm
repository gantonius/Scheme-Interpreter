...
(define (enumerate s)
  ...
  (define (enumerate-helper lst index)
    (if (null? lst)
      nil
      (cons (cons index (cons (car lst) nil)) (enumerate-helper (cdr lst) (+ index 1)))
    )
  )
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           ; BEGIN PROBLEM 19
           (cons (cons 'lambda (cons (car (zip values)) (map let-to-lambda body))) (let-to-lambda (cadr (zip values))))
           ; END PROBLEM 19
           ))
        (else
         ; BEGIN PROBLEM 19
